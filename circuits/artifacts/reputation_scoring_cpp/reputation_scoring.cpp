...
void ReputationScore_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void ReputationScore_3_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[4] = { 
Num2Bits_0_run,
LessThan_1_run,
GreaterEqThan_2_run,
ReputationScore_3_run };
...
uint get_size_of_witness() {return 69;}
...
ctx->componentMemory[coffset].templateName = "ReputationScore";
ctx->componentMemory[coffset].signalStart = soffset;
...
int cmp_index_ref_load = -1;
{
std::string new_cmp_name = "scoreCheck";
GreaterEqThan_2_create(mySignalStart+34,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
PFrElement aux_dest = &lvar[0];
...
// load src
Fr_mul(&expaux[1],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 2)],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 12)]); // line circom 37
Fr_add(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 23)],&expaux[1]); // line circom 37
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 12)]); // line circom 40
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
{
Fr_mul(&expaux[1],&signalValues[mySignalStart + 22],&lvar[0]); // line circom 47
{{
Fr_eq(&expaux[0],&signalValues[mySignalStart + 33],&expaux[1]); // line circom 47
}}
...
ReputationScore_3_create(1,0,ctx,"main",0);
ReputationScore_3_run(0,ctx);
}
